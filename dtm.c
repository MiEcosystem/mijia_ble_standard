...
static int uart_send(uint8_t const * const out, uint16_t len)
{
    for (int i = 0; i < len; i++) {
        USART_Tx(USART0, out[i]);
    }

    return 0;
}

static int uart_recv(uint8_t * const in, uint16_t ilen)
{
    int i;
    for (i = 0; i < ilen; i++) {
        in[i] = USART_Rx(USART0);
    }
    return i;
}
...
#if HAVE_OTP_PKI
    mi_mesh_otp_manufacture_init(uart_send, uart_recv, NULL, NULL);
#endif
